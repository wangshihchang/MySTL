...
class temporary_buffer {
   ...
   private:
    ...
    void initializer_buffer(const T&, std::true_type) {}
    void initializer_buffer(const T& value, std::false_type) {
...
temporary_buffer<ForwardIterator, T>::temporary_buffer(ForwardIterator first, ForwardIterator last) {
    try {
        ...
        allocate_buffer();
        if (len > 0)
            initializer_buffer(*first, std::is_trivially_default_constructible<T>());
    } catch (...) {
        free(buffer);
        buffer = nullptr;
        len = 0;
