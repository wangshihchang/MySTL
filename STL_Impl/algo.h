...
OutputIter adjacent_difference(InputIter first, InputIter last, OutputIter result, BinaryOperation binary_op) {
    if (first == last) return result;
    *result = *first;
    auto value = *first;
...
// 版本2
template <class InputIter, class OutputIter, class BinaryOperation>
OutputIter partial_sum(InputIter first, InputIter last, OutputIter result, BinaryOperation binary_op) {
    if (first == last) return result;
...
InputIter find_first_of(InputIter first1, InputIter last1, ForwardIter first2, ForwardIter last2, Compare comp) {
    for (; first1 != last1; ++first1) {
        for (auto iter = first2; iter != last2; ++iter) {
            if (comp(*iter, *first1)) return first1;
        }
    }
    return last1;
}
...
template <class RandomIter, class T>
MySTL::pair<RandomIter, RandomIter> erange_dispatch(RandomIter first, RandomIter last,
...
template <class ForwardIter, class T, class Comapre>
MySTL::pair<ForwardIter, ForwardIter> equal_range(ForwardIter first, ForwardIter last, const T& value, Comapre comp) {
...
/*****************************************************************************************/
// is_heap
// 检查[first, last)内的元素是否为一个最大堆，如果是，则返回 true
/*****************************************************************************************/
...
/*****************************************************************************************/
template <class T>
const T& median(const T& left, const T& middle, const T& right) {
    if (left < middle) {
        if (middle < right)
            return middle;
        else if (left < right)
...
const T& median(const T& left, const T& middle, const T& right, Comapre comp) {
    if (comp(left, middle)) {
        if (comp(middle, right))
            return middle;
...
/*****************************************************************************************/
template <class InputIter, class OuputIter, class T>
OuputIter remove_copy(InputIter first, InputIter last, OuputIter result, const T& value) {
    for (; first != last; ++first) {
        if (*first != value) {
            *result++ = *first;
        }
...
/*****************************************************************************************/
template <class InputIter, class OuputIter, class UnaryPredicate>
OuputIter remove_copy_if(InputIter first, InputIter last, OuputIter result, UnaryPredicate unary_pred) {
    for (; first != last; ++first) {
        if (!unary_pred(*first)) {
            *result++ = *first;
        }
...
RandomIter rotate_dispatch(RandomIter first, RandomIter middle, RandomIter last, random_access_iterator_tag) {
    ...
    auto cycle_times = MySTL::rgcd(n, l);
...
/*****************************************************************************************/
template <class ForwardIter, class OuputIter>
OuputIter rotate_copy(ForwardIter first, ForwardIter middle, ForwardIter last, OuputIter result) {
...
bool is_permutation_aux(ForwardIter1 first1, ForwardIter1 last1,
                        ForwardIter2 first2, ForwardIter2 last2, BinaryPred binary_pred) {
    ...
    for (; first1 != last1 && first2 != last2; ++first1, (void)++first2) {
        if (!binary_pred(*first1, *first2)) break;
    }
    if (is_ra_it) {
...
bool is_permutation(ForwardIter1 first1, ForwardIter1 last1,
                    ForwardIter2 first2, ForwardIter2 last2, BinaryPred binary_pred) {
    return MySTL::is_permutation_aux(first1, last1, first2, last2, binary_pred);
}
...
/*****************************************************************************************/
template <class InputIter1, class InputIter2, class OuputIter>
OuputIter merge(InputIter1 first1, InputIter1 last1,
                InputIter2 first2, InputIter2 last2, OuputIter result) {
    while (first1 != last1 && first2 != last2) {
        if (*first1 > *first2) {
            *result = *first2;
            ++first2;
...
// 重载版本使用函数对象 comp 代替比较操作
template <class InputIter1, class InputIter2, class OuputIter, class Comapre>
OuputIter merge(InputIter1 first1, InputIter1 last1,
                InputIter2 first2, InputIter2 last2,
...
pair<OutputIter1, OutputIter2> partition_copy(InputIter first, InputIter last,
...
template <class InputIter, class ForwardIter, class Compare>
ForwardIter unique_copy_dispatch(InputIter first, InputIter last,
                                 ForwardIter result, forward_iterator_tag, Compare comp) {
    *result = *first;
    while (++first != last) {
